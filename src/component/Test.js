  ...
  for (let i=0; i<4; i++) { // 添加4张待加载的图片
    ...
    images.push( // 新建 img jsx 放入 images （图片地址不放入 src 而是放入 自定义属性 data-src）
        <div style={css.imageBox} key={i}>
          <img ref={ ref } data-src={`https://pschina.github.io/src/assets/images/${i}.jpg`} style={{border:'1px solid #000'}}/>
        </div>
    )
  }

  const threshold = [0.5] // 这是触发时机 0.01代表出现 1%的面积出现在可视区触发一次回掉函数 threshold = [0, 0.25, 0.5, 0.75]  表示分别在0% 25% 50% 75% 时触发回掉函数
